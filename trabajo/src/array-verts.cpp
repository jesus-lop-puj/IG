...
void DescrTabla::activar_md()
{
   ...
   // ......
   //Si el VBO no está creado
   if(nombre_vbo==0){
     glGenBuffers(1, &nombre_vbo);  //Se crea el VBO
     glBindBuffer(tipo_tabla, nombre_vbo);  //Se activa el VBO
     glBufferData(tipo_tabla, tamano_en_bytes, datos, GL_STATIC_DRAW); //Se reserva memoria en la GPU
   }
   else{
...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva )
{
   ...
   for(GLuint i=0; i<nv; i++){     //Recorremos los vértices
     ...
     //Enviamos los atributos del vertice
     if(colores != nullptr){
     ...
     if(coords_textura != nullptr){
      ...
      //Enviamos las coordenadas del vertice
...
void ArrayVertices::visualizarGL_MI_DAE( const GLenum tipo_primitiva )
{
   ...
   glBindVertexArray(0);  //Activamos VAO por defecto
   deshabilitar_tablas();
   coordenadas->activar_mi(); //Activamos la tabla de coordenadas

   //Activamos las tablas de atributos que corresponde
   if(colores != nullptr){
     colores->activar_mi();
   }
   if(normales != nullptr){
   ...
   //Invocamos en funcion de si hay indices
   if(indices != nullptr){
...
void ArrayVertices::visualizarGL_MD_VAO( const GLenum tipo_primitiva )
{
   ...
   if(nombre_vao==0){ //Si el VAO no esta creado
     glGenVertexArrays(1, &nombre_vao); //Se crea
     glBindVertexArray(nombre_vao); //Se activa
   ...
   else{  //Si ya estaba creado
    glBindVertexArray(nombre_vao);
   }

     //Depende de si hay indices o no invocamos
     if(indices!=nullptr){
     ...
     glBindVertexArray(0); //Deshabilitamos el VAO
